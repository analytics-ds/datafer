...
export default async function AgencyFolderDetail({
  params,
}: {
  ...
  const db = getDb();
  const [row] = await db
    .select({ client, ownerName: user.name })
    .from(client)
    .leftJoin(user, eq(user.id, client.ownerId))
    .where(and(eq(client.id, id), eq(client.scope, "agency")))
    .limit(1);

  if (!row) notFound();
  const folder = row.client;
  ...
  return (
    <div className="px-10 py-10 max-w-[1100px]">
      ...
      {briefs.length === 0 ? (
        <EmptyState
          title="Aucun brief partagé"
          description="Crée un brief dans ce dossier pour le partager avec l'équipe."
