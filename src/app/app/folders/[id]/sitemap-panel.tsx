...
export function SitemapPanel({
  ...
  async function resync() {
    if (!sitemapUrl.trim()) {
      setError(t("sitemap.error.noUrl"));
      return;
    }
    setResyncing(true);
    setStatus("syncing");
  ...
  return (
    <div className="mb-6 rounded-[var(--radius)] border border-[var(--border)] bg-[var(--bg-card)] p-5">
      <div className="flex items-center justify-between gap-3 mb-3">
        <div>
          <h3 className="text-[13px] font-semibold text-[var(--text)]">{t("maillage.title")}</h3>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <input
          type="url"
          placeholder={t("sitemap.placeholder")}
          value={sitemapUrl}
        ...
        >
          {resyncing ? t("sitemap.syncing") : urlCount === 0 ? t("sitemap.index") : t("sitemap.resync")}
        </button>
      </div>

      {savedMsg && <p className="mt-2 text-[12px] text-emerald-600">{savedMsg}</p>}
      {error && <p className="mt-2 text-[12px] text-red-600">{t("sitemap.error", { error })}</p>}
    </div>
  );
}
  ...
  if (urlCount === 0) {
    return (
      <span className="rounded-[var(--radius-pill)] bg-[var(--bg-warm)] px-3 py-1 text-[11px] font-semibold text-[var(--text-muted)]">
        {t("sitemap.status.empty")}
      </span>
    );
  }
  const when = !lastSyncAt ? t("sitemap.never") : mounted ? timeAgo(lastSyncAt, t) : t("sitemap.recent");
  return (
    <span className="rounded-[var(--radius-pill)] bg-emerald-50 px-3 py-1 text-[11px] font-semibold text-emerald-700">
      {t("sitemap.urlCount", { count: urlCount, when })}
    </span>
  );
}

function timeAgo(d: Date, t: Translator): string {
