...
export function SharePanel({
  ...
  function enable() {
    startTransition(async () => {
      const res = await enableShareAction(folderId);
  ...
  function revoke() {
    startTransition(async () => {
      const res = await revokeShareAction(folderId);
  ...
  return (
    <div ref={wrapperRef} className="relative inline-block">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 z-20 bg-[var(--bg-card)] border border-[var(--border)] rounded-[var(--radius-sm)] shadow-[var(--shadow-lg)] p-5 pt-9 w-[400px]">
          <button
            type="button"
